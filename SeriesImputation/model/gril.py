...
class GRIL(nn.Module):
    def __init__(self,
        ...
        for i in range(self.n_layers):
            self.cells.append(MpGruUnit(d_in=rnn_input_size if i == 0 else self.hidden_size,
...
class BiGRIL(nn.Module):
    ...
    def forward(self, x, adj, mask=None, u=None, cached_support=False):
        if cached_support and (self.supp is not None):
            ...
            supp = MessagePassGates.compute_support(adj, x.device)
...
#expecting input data shape :[batch_size,t,Nt,d]
# class GRIL(pl.LightningModule):
#     def __init__(self,
#                 input_sizes,
#                 output_sizes,
#                 layer_num=1,
...
#         super(GRIL,self).__init__()
#         #self.mp_layer=MessagePassGates(input_sizes,output_sizes,hidden_sizes=[common_hidden_size])
#         self.decoder=Decoder(input_sizes,output_sizes,final_hidden_size)
#         self.encoder=MpGruUnit(input_sizes,output_sizes,[common_hidden_size],layer_num)
    
#     def forward(self,X,M,W):
#         """
#         shape of X:(batch_size,t,Nt,d) where t is the time lenght,Nt is num of nodes,d is dimension of a single node
#         shape of M:(batch_size,t,Nt,d) the mask stands for missing value
#         shape of W:(batch_size,t,Nt,Nt) means the weight on edges in graph
#         """
#         serieslength=X.shape[1]
#         #init hidden states
#         hiddenlist=list()
#         H0=torch.zeros((*X.shape[:3],))
#         hiddenlist.append(H0)
#         #encoding for hidden states
#         for t in range(1,serieslength):
#             Xt=X[:,t,:,:].squeeze(1)
#             Mt=M[:,t,:,:].squeeze(1)
#             Wt=W[:,t,:,:].squeeze(1)
#             Ht_1=hiddenlist[t-1].squeeze(1)
#             Ht=self.encoder(Xt,Mt,Ht_1,Wt)
#             hiddenlist.append(Ht)

    
#         H=torch.stack(hiddenlist,dim=0)
#         H=H.permute(1,0,2,3)
#         #decoding for value imputation
#         reconstructed_X=self.decoder(X,H,W)
