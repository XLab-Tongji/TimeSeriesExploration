...
#based on official example by torch_geometric
epsilon=1e-8

class MessagePassGates(nn.Module):
    """
    ...
    def __init__(self, c_in, c_out, support_len=3, order=2, include_self=True):
        super(MessagePassGates, self).__init__()
    ...
    def compute_support_orderK(adj, k, include_self=False, device=None):
        if isinstance(adj, (list, tuple)):
            ...
            support = MessagePassGates.compute_support(adj, device)
        supp_k = []
        for a in support:
            ak = a
            for i in range(k - 1):
