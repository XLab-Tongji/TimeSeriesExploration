...
datasets_path=os.path.join(os.path.abspath(os.path.dirname(current_path)),'data\pems-bay')
...
class PemsBay(PandasDataset):
    ...
    def load(self, impute_zeros=True):
        path = os.path.join(datasets_path, 'pems_bay.h5')
        df = pd.read_hdf(path)
    ...
    def load_distance_matrix(self, ids):
        path = os.path.join(datasets_path, 'pems_bay_dist.npy')
        try:
            dist = np.load(path)
        except:
            distances = pd.read_csv(os.path.join(datasets_path, 'distances_bay.csv'))
            num_sensors = len(ids)
