...
datasets_path=os.path.join(os.path.abspath(os.path.dirname(current_path)),'data\metr-la')
...
class MetrLA(PandasDataset):
    ...
    def load(self, impute_zeros=True):
        path = os.path.join(datasets_path, 'metr_la.h5')
        df = pd.read_hdf(path)
    ...
    def load_distance_matrix(self):
        ...
        try:
            ...
            distances = pd.read_csv(os.path.join(datasets_path, 'distances_la.csv'))
            with open(os.path.join(datasets_path, 'sensor_ids_la.txt')) as f:
